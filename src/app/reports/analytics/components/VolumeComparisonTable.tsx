...
import type { ReportTypeRow } from "@/lib/data";
...
type SortKey = "label" | "created" | "readyRate" | "matchRate";
        ...
        <table className="-mb-px w-full">
          <thead>
            <tr className="border-b border-[var(--color-border)]">
              ...
              <SortableTh onClick={() => handleSort("readyRate")} align="right" sortIcon={<SortIcon active={sortKey === "readyRate"} dir={sortDir} />}>Ready Rate</SortableTh>
          ...
          <tbody>
            {sorted.map((row) => (
              <tr key={row.typeKey} className="border-b border-[var(--color-border)]">
                ...
                <td className="px-4 py-2.5 text-right">
                  <span className="text-sm text-[var(--color-text)]">{row.readyRate}%</span>
                </td>
                <td className="px-4 py-2.5 text-right">
