# Understanding Business Registry Verification results[](#understanding-business-registry-verification-results)

## Overview[](#overview)

The Business Registry Verification (BRV) is used in [KYB](./3OGe9IwySJmDvXL2DOfV7N.md) to verify the existence of a business within an authoritative or government data source. For more details, see the [Business Registry Verification overview](./3pkLBWKg5wIQJru7PATxBc.md).
...
## Example Business Registry Verification result[](#example-business-registry-verification-result)
...
![BusinessRegistryVerification](../images/BusinessRegistryVerification_537602121756.png)
...
![BusinessRegistryVerificationRawInput](../images/BusinessRegistryVerificationRawInput_b30911cd351d.png)
...
In this example, we see that two Verification checks were [required](./7IAy61dAMRDQ1Q77ugwpcA.md) (indicated by blue checkmarks in the Required column). All of the checks **passed.** While reviewing non-required Verification checks is common practice, consider updating your Verification Template to require any check whose failure would be unacceptable. This ensures future Verification runs will properly evaluate these critical checks.
...
### Identity Comparison Check for Business Registry Verification[](#identity-comparison-check-for-business-registry-verification)

![BusinessRegistryVerificationIdentityVerif](../images/BusinessRegistryVerificationIdentityVerif_b7dc71f5769c.png)
...
-   **Match result = `Full`**: Business Name, Address Street 1, Address City, Address Subdivision, Address Postal code all were a `Full` match. This means the value the user submitted matched exactly what was in a known database.
...
### Identity Comparison Check with Match Groups for Business Registry Verification[](#identity-comparison-check-with-match-groups-for-business-registry-verification)
...
For more details, see [Understanding match groups](./1gZBzL78LmIZblpOSJNpW6.md).
