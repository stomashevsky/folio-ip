...
Changes, Fixes, and Features for our Android SDK.

## Releases

Watch [this repo](https://github.com/persona-id/inquiry-android-2) to be notified when new versions of the android SDK are available.
...
#### Best practices for keeping the Android SDK updated

Keeping your SDK up to date is essential for ensuring your app remains secure, performant, and compatible with the latest Android versions and Persona features. Here are a few best practices we recommend:
...
-   📬 **Subscribe to SDK updates** by watching [persona-id/inquiry-android-2](https://github.com/persona-id/inquiry-android-2).
    
    > Note: The Android SDK is distributed via Maven. This GitHub repo exists only to provide changelogs and release tracking.
...
-   Fixed a bug where the error message for required input was not clea red after the input was entered.
...
-   Fixed a rare native library crash where the native library could not be found due to an incompatible architecture.
