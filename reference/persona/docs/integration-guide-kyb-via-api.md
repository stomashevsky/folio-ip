...
We recommend listening to the `transaction.status-updated` webhook ([docs](./webhooks.md)) to know when to proceed with the business. You can alternatively make an API call to fetch the transaction ([docs](./api-reference/transactions/retrieve-a-transaction.md))
