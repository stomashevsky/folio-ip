...
[February 24, 2025](./24.md)

[February 24, 2025](./24.md)

### Inquiries

-   🌱 **Add Mark Inquiry For Review endpoint**: You can now mark an Inquiry for review using the [Mark an Inquiry for manual review](../../../../inquiries/mark-an-inquiry-for-review.md) API.
-   🍃 **Add `ip-connection-type` attribute to Inquiry Sessions**: [Inquiry Session resources](../../../../inquiry-sessions/retrieve-an-inquiry-session.md) now include a `ip-connection-type` string attribute. Possible values are `mobile`, `datacenter`, `residential`, `corporate`, and `education`.
